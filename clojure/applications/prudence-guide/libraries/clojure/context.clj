...
(defn get-context-attribute [name get-default-value]
	(let [value (.. executable getContainer getResource getContext getAttributes (get name))]
		(if (nil? value)
			(let [value (get-default-value)]
				...
				(let [existing (.. executable getContainer getResource getContext getAttributes (putIfAbsent name value))]
					(if (nil? existing)
						value
						existing)))
			value)))
